          ...
          <div className="motion-reveal">
            ...
            <div className="mt-10 space-y-5">
              {skillCategories.map((category) => (
                <div key={category.key} className="surface-card p-6">
                  <h3 className="text-sm font-semibold text-white">{t(`categories.${category.key}`)}</h3>
                  <div className="mt-5 flex flex-wrap gap-2">
                    {category.skills.map((skill) => (
                      <span key={skill} className="rounded-full border border-[var(--border-soft)] bg-[var(--accent-soft)] px-3 py-1.5 text-xs text-[var(--text-soft)]">{skill}</span>
                    ))}
                  </div>
                </div>
              ))}
